...
const reminders = [
  { id: '1', type: 'medicine' as const, label: 'Uống thuốc', scheduleLabel: 'Bạn cần uống thuốc vào', name: 'Paracetamol 500mg', date: new Date(2026, 4, 21), color: 'bg-orange-500' },
  { id: '2', type: 'vaccine' as const, label: 'Tiêm chủng', scheduleLabel: 'Bạn có lịch tiêm vào', name: 'Vắc-xin Cúm mùa', date: new Date(2026, 4, 23), color: 'bg-emerald-600' },
  { id: '3', type: 'revisit' as const, label: 'Tái khám', scheduleLabel: 'Bạn có lịch tái khám vào', name: 'Tim mạch — BV Bạch Mai', date: new Date(2026, 4, 24), color: 'bg-blue-600' },
]

const trackingRecords = [
  { id: '1', dateObj: new Date(2025, 2, 12), date: '12/03/2025', disease: 'Huyết áp cao', place: 'BV Bạch Mai', medicine: 'Amlodipine 5mg', tag: 'Đang theo dõi', tagColor: 'bg-blue-100 text-blue-700' },
  { id: '2', dateObj: new Date(2025, 0, 5), date: '05/01/2025', disease: 'Tiểu đường type 2', place: 'PK Đa khoa Hà Nội', medicine: 'Metformin 500mg', tag: 'Đang theo dõi', tagColor: 'bg-blue-100 text-blue-700' },
  { id: '3', dateObj: new Date(2024, 8, 17), date: '17/09/2024', disease: 'Đau cổ vai gáy', place: 'BV Đa khoa Tâm Anh', medicine: 'Ibuprofen 400mg', tag: 'Đang theo dõi', tagColor: 'bg-blue-100 text-blue-700' },
...
const REPEAT_OPTIONS = ['Không lặp','Mỗi ngày','Các ngày trong tuần','Cuối tuần','Mỗi tuần','Mỗi tháng','Mỗi năm']
...
type PrescriptionView = { disease: string; date: string; place: string; doctor?: string; medicine: string }

const mockExtras: Record<string, Pick<DetailRecord, 'images' | 'links'>> = {
  h2: {
    images: [
      { id: 'i1', label: 'X-quang tim phổi', type: 'xray' },
      { id: 'i2', label: 'Siêu âm tim', type: 'ultrasound' },
    ],
    links: [{ id: 'l1', label: 'Kết quả xét nghiệm máu 12/03/2025' }],
  },
  h3: {
    images: [{ id: 'i3', label: 'Xét nghiệm HbA1c', type: 'other' }],
    links: [],
  },
}
...
export const YhHomePage = () => {
  ...
  const [activeSection, setActiveSection] = useState<'tracking' | 'history'>('tracking')
  const [deleteIndex, setDeleteIndex] = useState<number | null>(null)
  const [slide, setSlide] = useState(0)
  const [removedIds, setRemovedIds] = useState<Set<string>>(new Set())
  const [unfollowId, setUnfollowId] = useState<string | null>(null)
  const [historyFollowSet, setHistoryFollowSet] = useState<Set<string>>(new Set())
  const [followConfirmId, setFollowConfirmId] = useState<string | null>(null)
  const [unfollowHistoryId, setUnfollowHistoryId] = useState<string | null>(null)
  ...
  const [specialView, setSpecialView] = useState<'hidden' | 'deleted' | null>(null)
  ...
  const [prescriptionView, setPrescriptionView] = useState<PrescriptionView | null>(null)
  ...
  const activeHistory = historyRecords.filter(r => !hiddenIds.has(r.id) && !deletedIds.has(r.id))
    ...
    <YhLayout activeTab='home'>
      ...
      {unfollowId !== null && (
        <View className='absolute inset-0 z-50 items-center justify-center bg-black/40 px-6'>
          <View className='w-full rounded-2xl bg-white p-5 shadow-2xl'>
            <Span className='mb-1 text-center text-sm font-semibold text-gray-800'>Bỏ theo dõi</Span>
            <Span className='mb-5 text-center text-xs text-gray-500'>Bạn chắc chắn muốn bỏ theo dõi hồ sơ sức khỏe này?</Span>
            <View className='flex-row gap-3'>
              <Pressable
                onPress={() => {
                  ...
                  if (id.startsWith('copy_')) {
                    ...
                    setHistoryFollowSet(prev => { const next = new Set(prev); next.delete(origId); return next })
                  } else {
              ...
              <Pressable onPress={() => setUnfollowId(null)} className='flex-1 items-center rounded-xl py-3' style={{ backgroundColor: PRIMARY }}>
      ...
      {showSectionMore && (
        <View className='absolute inset-0 z-50 justify-end bg-black/30'>
          ...
          <View className='rounded-t-3xl bg-white p-4'>
            ...
            {[
              { label: 'Xuất PDF', icon: <svg width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='#132C95' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'><path d='M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z'/><polyline points='14 2 14 8 20 8'/><line x1='16' y1='13' x2='8' y2='13'/><line x1='16' y1='17' x2='8' y2='17'/><polyline points='10 9 9 9 8 9'/></svg>, action: () => { setBulkMode('pdf'); setSelectedIds(new Set()); setShowSectionMore(false) } },
              { label: 'Chia sẻ', icon: <svg width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='#132C95' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'><circle cx='18' cy='5' r='3'/><circle cx='6' cy='12' r='3'/><circle cx='18' cy='19' r='3'/><line x1='8.59' y1='13.51' x2='15.42' y2='17.49'/><line x1='15.41' y1='6.51' x2='8.59' y2='10.49'/></svg>, action: () => { setBulkMode('share'); setSelectedIds(new Set()); setShowSectionMore(false) } },
              { label: 'Bị ẩn', icon: <svg width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='#6b7280' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'><path d='M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24'/><line x1='1' y1='1' x2='23' y2='23'/></svg>, action: () => { setSpecialView('hidden'); setShowSectionMore(false) } },
              { label: 'Đã xóa', icon: <svg width='16' height='16' viewBox='0 0 24 24' fill='none' stroke='#ef4444' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'><polyline points='3 6 5 6 21 6'/><path d='M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6'/><path d='M10 11v6'/><path d='M14 11v6'/><path d='M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2'/></svg>, action: () => { setSpecialView('deleted'); setShowSectionMore(false) } },
            ].map(item => (
              <Pressable key={item.label} onPress={item.action} className='flex-row items-center gap-3 rounded-xl px-3 py-3'>
                {item.icon}
        ...
        <View className='absolute inset-0 z-50 items-center justify-center bg-black/40 px-6'>
          <View className='w-full rounded-2xl bg-white p-5 shadow-2xl'>
            ...
            <Span className='mb-5 text-center text-xs text-gray-500'>
              Bạn có muốn {bulkMode === 'pdf' ? 'xuất PDF' : 'chia sẻ'} {selectedIds.size} hồ sơ sức khỏe này không?
            </Span>
            <View className='flex-row gap-3'>
              <Pressable onPress={() => { setShowBulkConfirm(false); setBulkMode(null); setSelectedIds(new Set()) }} className='flex-1 items-center rounded-xl py-3' style={{ backgroundColor: PRIMARY }}>
                <Span className='text-sm font-semibold text-white'>Đồng ý</Span>
              </Pressable>
              <Pressable onPress={() => setShowBulkConfirm(false)} className='flex-1 items-center rounded-xl border border-gray-200 bg-white py-3'>
      ...
      {recordAction !== null && (
        <View className='absolute inset-0 z-50 items-center justify-center bg-black/40 px-6'>
          <View className='w-full rounded-2xl bg-white p-5 shadow-2xl'>
            <Span className='mb-1 text-center text-sm font-semibold text-gray-800'>
              {recordAction.type === 'pdf' ? 'Xuất PDF' : recordAction.type === 'share' ? 'Chia sẻ' : recordAction.type === 'hide' ? 'Ẩn hồ sơ' : 'Xóa hồ sơ'}
            </Span>
            <Span className='mb-5 text-center text-xs text-gray-500'>
              {recordAction.type === 'pdf' && 'Bạn có muốn xuất PDF hồ sơ này không?'}
              {recordAction.type === 'share' && 'Bạn có muốn chia sẻ hồ sơ này không?'}
              {recordAction.type === 'hide' && 'Bạn có muốn ẩn hồ sơ này không?'}
              {recordAction.type === 'delete' && 'Bạn có muốn xóa hồ sơ này không?'}
            </Span>
            <View className='flex-row gap-3'>
              <Pressable
                onPress={() => {
                  if (recordAction.type === 'hide') setHiddenIds(prev => { const n = new Set(prev); n.add(recordAction.id); return n })
                  if (recordAction.type === 'delete') setDeletedIds(prev => { const n = new Set(prev); n.add(recordAction.id); return n })
                  setRecordAction(null)
                }}
                className='flex-1 items-center rounded-xl py-3'
                style={{ backgroundColor: recordAction.type === 'delete' ? '#ef4444' : PRIMARY }}
              >
                <Span className='text-sm font-semibold text-white'>Đồng ý</Span>
              </Pressable>
              <Pressable onPress={() => setRecordAction(null)} className='flex-1 items-center rounded-xl border border-gray-200 bg-white py-3'>
                <Span className='text-sm text-gray-600'>Hủy bỏ</Span>
              </Pressable>
            </View>
          </View>
      ...
      {restoreId !== null && (
        <View className='absolute inset-0 z-50 items-center justify-center bg-black/40 px-6'>
          <View className='w-full rounded-2xl bg-white p-5 shadow-2xl'>
            <Span className='mb-1 text-center text-sm font-semibold text-gray-800'>Khôi phục hồ sơ</Span>
            <Span className='mb-5 text-center text-xs text-gray-500'>Bạn có muốn khôi phục hồ sơ sức khỏe này không?</Span>
            <View className='flex-row gap-3'>
              <Pressable
                onPress={() => {
                  setHiddenIds(prev => { const n = new Set(prev); n.delete(restoreId!); return n })
                  setDeletedIds(prev => { const n = new Set(prev); n.delete(restoreId!); return n })
                  setRestoreId(null)
                }}
                className='flex-1 items-center rounded-xl py-3'
                style={{ backgroundColor: PRIMARY }}
              >
                <Span className='text-sm font-semibold text-white'>Khôi phục</Span>
              </Pressable>
              <Pressable onPress={() => setRestoreId(null)} className='flex-1 items-center rounded-xl border border-gray-200 bg-white py-3'>
                <Span className='text-sm text-gray-600'>Hủy bỏ</Span>
              </Pressable>
            </View>
          </View>
            ...
            <ScrollView>
              <View className='p-4'>
                ...
                {[...new Set(historyRecords.map(r => r.disease))].map(d => (
                  <Pressable key={d} onPress={() => setFilterDiseases(prev => { const n = new Set(prev); n.has(d) ? n.delete(d) : n.add(d); return n })} className='mb-2 flex-row items-center gap-2'>
                    <View className='h-4 w-4 items-center justify-center rounded-sm border' style={{ borderColor: filterDiseases.has(d) ? PRIMARY : '#d1d5db', backgroundColor: filterDiseases.has(d) ? PRIMARY : '#fff' }}>
                      {filterDiseases.has(d) && <svg width='10' height='10' viewBox='0 0 24 24' fill='none' stroke='#fff' strokeWidth='3' strokeLinecap='round'><polyline points='20 6 9 17 4 12'/></svg>}
                    </View>
                    <Span className='text-xs text-gray-700'>{d}</Span>
                  </Pressable>
                ))}
                {/* Bac si */}
                <Span className='mb-2 mt-3 text-[10px] font-semibold text-gray-400'>BÁC SĨ</Span>
                {[...new Set(historyRecords.map(r => r.doctor))].map(doc => (
                  <Pressable key={doc} onPress={() => setFilterDoctors(prev => { const n = new Set(prev); n.has(doc) ? n.delete(doc) : n.add(doc); return n })} className='mb-2 flex-row items-center gap-2'>
                    <View className='h-4 w-4 items-center justify-center rounded-sm border' style={{ borderColor: filterDoctors.has(doc) ? PRIMARY : '#d1d5db', backgroundColor: filterDoctors.has(doc) ? PRIMARY : '#fff' }}>
                      {filterDoctors.has(doc) && <svg width='10' height='10' viewBox='0 0 24 24' fill='none' stroke='#fff' strokeWidth='3' strokeLinecap='round'><polyline points='20 6 9 17 4 12'/></svg>}
                    </View>
                    <Span className='text-xs text-gray-700'>{doc}</Span>
                  </Pressable>
                ))}
                {/* Noi kham */}
                <Span className='mb-2 mt-3 text-[10px] font-semibold text-gray-400'>NƠI KHÁM</Span>
                {[...new Set(historyRecords.map(r => r.place))].map(p => (
                  <Pressable key={p} onPress={() => setFilterPlaces(prev => { const n = new Set(prev); n.has(p) ? n.delete(p) : n.add(p); return n })} className='mb-2 flex-row items-center gap-2'>
                    <View className='h-4 w-4 items-center justify-center rounded-sm border' style={{ borderColor: filterPlaces.has(p) ? PRIMARY : '#d1d5db', backgroundColor: filterPlaces.has(p) ? PRIMARY : '#fff' }}>
                      {filterPlaces.has(p) && <svg width='10' height='10' viewBox='0 0 24 24' fill='none' stroke='#fff' strokeWidth='3' strokeLinecap='round'><polyline points='20 6 9 17 4 12'/></svg>}
                    </View>
      ...
      {followConfirmId !== null && (
        <View className='absolute inset-0 z-50 items-center justify-center bg-black/40 px-6'>
          <View className='w-full rounded-2xl bg-white p-5 shadow-2xl'>
            <Span className='mb-1 text-center text-sm font-semibold text-gray-800'>Theo dõi hồ sơ</Span>
            <Span className='mb-5 text-center text-xs text-gray-500'>Bạn có muốn theo dõi hồ sơ sức khỏe này không?</Span>
            <View className='flex-row gap-3'>
              <Pressable
                onPress={() => {
                  setHistoryFollowSet(prev => { const next = new Set(prev); next.add(followConfirmId!); return next })
              ...
              <Pressable onPress={() => setFollowConfirmId(null)} className='flex-1 items-center rounded-xl border border-gray-200 bg-white py-3'>
      ...
      {unfollowHistoryId !== null && (
        <View className='absolute inset-0 z-50 items-center justify-center bg-black/40 px-6'>
          <View className='w-full rounded-2xl bg-white p-5 shadow-2xl'>
            <Span className='mb-1 text-center text-sm font-semibold text-gray-800'>Bỏ theo dõi</Span>
            <Span className='mb-5 text-center text-xs text-gray-500'>Bạn có chắc bỏ theo dõi hồ sơ sức khỏe này không? Nếu đồng ý, hồ sơ này sẽ không còn được theo dõi bên tính năng Theo dõi sức khỏe nữa.</Span>
            <View className='flex-row gap-3'>
              <Pressable
                onPress={() => {
                  setHistoryFollowSet(prev => { const next = new Set(prev); next.delete(unfollowHistoryId!); return next })
              ...
              <Pressable onPress={() => setUnfollowHistoryId(null)} className='flex-1 items-center rounded-xl py-3' style={{ backgroundColor: PRIMARY }}>
                <Span className='text-sm font-semibold text-white'>Hủy bỏ</Span>
              </Pressable>
            </View>
          </View>
      ...
      {deleteIndex !== null && (
        <View className='absolute inset-0 z-50 items-center justify-center bg-black/40 px-6'>
          <View className='w-full rounded-2xl bg-white p-5 shadow-2xl'>
            <Span className='mb-1 text-center text-sm font-semibold text-gray-800'>Xóa nhắc nhở</Span>
            <Span className='mb-5 text-center text-xs text-gray-500'>Bạn có chắc chắn muốn xóa nhắc nhở này không?</Span>
            <View className='flex-row gap-3'>
              <Pressable onPress={() => setDeleteIndex(null)} className='flex-1 items-center rounded-xl bg-orange-500 py-3'>
                <Span className='text-sm font-semibold text-white'>Không</Span>
              </Pressable>
              <Pressable onPress={() => setDeleteIndex(null)} className='flex-1 items-center rounded-xl border border-blue-600 bg-white py-3'>
                <Span className='text-sm font-semibold text-blue-600'>Xác nhận</Span>
              </Pressable>
            </View>
          </View>
        </View>
              ...
              {calCells.map((day, idx) => (
                <View key={idx} className='items-center justify-center' style={{ width: '14.28%', paddingVertical: 4 }}>
                  {day ? (
                    ...
                    >
                      <Span className='text-[11px]' style={{ color: isCalSelected(calYear, calMonth, day) ? '#fff' : '#1a1a2e', fontWeight: isCalSelected(calYear, calMonth, day) ? '600' : '400' }}>{day}</Span>
                    </Pressable>
      ...
      {showReminder && (
        <View className='absolute inset-0 z-50 items-center justify-end bg-black/40'>
          <View className='w-full rounded-t-3xl bg-white p-4'>
            <View className='mb-4 flex-row items-center justify-between'>
              <Pressable onPress={() => setShowReminder(false)}><Span className='text-sm text-gray-500'>Hủy</Span></Pressable>
              <Span className='text-sm font-semibold text-gray-800'>Nhắc uống thuốc</Span>
              <Pressable onPress={() => { setEditForm(f => ({ ...f, reminder: `${String(remHour).padStart(2,'0')}:${String(remMin).padStart(2,'0')} - ${remRepeat}` })); setShowReminder(false) }}>
            ...
            <View className='rounded-2xl bg-slate-50 overflow-hidden'>
        ...
        <View className='absolute inset-0 z-[60] items-center justify-center bg-black/70 px-5'>
          <View className='w-full overflow-hidden rounded-2xl shadow-2xl'>
            ...
            <View className='items-center px-4 py-3' style={{ backgroundColor: PRIMARY }}>
              ...
              <Span className='mt-0.5 text-xs font-bold text-white'>{prescriptionView.place}</Span>
            </View>
            ...
            <View className='bg-white px-4 py-4'>
              ...
              <View className='mb-3 rounded-xl bg-slate-50 p-3'>
                <View className='mb-1.5 flex-row gap-2'>
                  <Span className='w-16 text-[10px] font-semibold text-gray-400'>Ngay ke:</Span>
                  <Span className='text-[10px] text-gray-700'>{prescriptionView.date}</Span>
                </View>
                <View className='mb-1.5 flex-row gap-2'>
                  <Span className='w-16 text-[10px] font-semibold text-gray-400'>Chan doan:</Span>
                  <Span className='flex-1 text-[10px] text-gray-700'>{prescriptionView.disease}</Span>
                </View>
                {prescriptionView.doctor && (
                  <View className='flex-row gap-2'>
                    <Span className='w-16 text-[10px] font-semibold text-gray-400'>Bac si:</Span>
                    <Span className='text-[10px] text-gray-700'>{prescriptionView.doctor}</Span>
                  </View>
                )}
              </View>
              ...
              <View className='mb-4 flex-row items-start gap-2 rounded-xl bg-orange-50 p-3'>
                ...
                <View className='flex-1'>
                  <Span className='text-[11px] font-semibold text-orange-800'>{prescriptionView.medicine}</Span>
              ...
              <View className='items-end'>
                ...
                <Span className='text-[9px] text-gray-500'>{prescriptionView.doctor ?? 'Bac si dieu tri'}</Span>
              </View>
            </View>
          </View>
      ...
      {detailRecord !== null && (
        <View className='absolute inset-0 z-50 flex-col bg-slate-50'>
          ...
          <View className='flex-row items-center border-b border-gray-100 bg-white px-4 py-3'>
            <Pressable onPress={() => setDetailRecord(null)} className='mr-3 h-8 w-8 items-center justify-center rounded-full bg-gray-100'>
          ...
          <ScrollView className='flex-1 px-4 py-4'>
            ...
            <View className='mb-3 rounded-2xl bg-white p-4 shadow-sm'>
              <Span className='mb-1 text-base font-bold text-gray-900'>{detailRecord.disease}</Span>
              <View className='mb-3 flex-row items-center gap-1.5'>
                <svg width='11' height='11' viewBox='0 0 24 24' fill='none' stroke='#9ca3af' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
                  <rect x='3' y='4' width='18' height='18' rx='2'/><line x1='16' y1='2' x2='16' y2='6'/><line x1='8' y1='2' x2='8' y2='6'/><line x1='3' y1='10' x2='21' y2='10'/>
                </svg>
                <Span className='text-[11px] text-gray-400'>{detailRecord.date}</Span>
                <Span className='text-[11px] text-gray-300'>|</Span>
                <svg width='11' height='11' viewBox='0 0 24 24' fill='none' stroke='#9ca3af' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
                  <path d='M21 10c0 7-9 13-9 13S3 17 3 10a9 9 0 0 1 18 0z'/><circle cx='12' cy='10' r='3'/>
                </svg>
                <Span className='text-[11px] text-gray-400'>{detailRecord.place}</Span>
              </View>
              {detailRecord.doctor && (
                <View className='flex-row items-center gap-1.5'>
                  ...
                  <Span className='text-[11px] text-gray-400'>{detailRecord.doctor}</Span>
                </View>
              )}
            </View>
            ...
            <View className='mb-3 rounded-2xl bg-orange-50 p-4'>
              ...
              <Span className='text-sm font-medium text-orange-800'>{detailRecord.medicine}</Span>
            </View>

            {/* Images & Documents */}
            <View className='mb-3 rounded-2xl bg-white p-4 shadow-sm'>
              <View className='mb-3 flex-row items-center justify-between'>
                <Span className='text-xs font-semibold text-gray-700'>Hình ảnh & Tài liệu</Span>
              ...
              {(detailRecord.images && detailRecord.images.length > 0) ? (
                  ...
                  {detailRecord.images.map(img => (
                    <View
                      ...
                      style={{
                        width: '47%',
                        height: 88,
                        backgroundColor: img.type === 'xray' ? '#1e293b' : img.type === 'ultrasound' ? '#1e3a5f' : '#374151',
                      }}
                    >
                      ...
                      <Span className='mt-1.5 text-[9px] font-medium text-white/70'>{img.label}</Span>
                    </View>
                  ))}
                </View>
              ) : (
                <View className='items-center justify-center rounded-xl border border-dashed border-gray-200 py-5'>
                  ...
                  <Span className='mt-2 text-[10px] text-gray-300'>Chưa có hình ảnh</Span>
                </View>
              )}
            </View>
            ...
            <View className='mb-4 rounded-2xl bg-white p-4 shadow-sm'>
              <View className='mb-3 flex-row items-center justify-between'>
                <Span className='text-xs font-semibold text-gray-700'>Liên kết</Span>
                <Pressable className='flex-row items-center gap-1 rounded-full bg-blue-50 px-2 py-0.5'>
                  <Span className='text-[10px] font-medium' style={{ color: PRIMARY }}>+ Thêm</Span>
                </Pressable>
              </View>
              {(detailRecord.links && detailRecord.links.length > 0) ? (
                detailRecord.links.map(link => (
                  <Pressable key={link.id} className='mb-2 flex-row items-center gap-2 rounded-xl border border-gray-100 bg-slate-50 px-3 py-2.5 last:mb-0'>
                    ...
                    <Span className='flex-1 text-[11px] text-gray-700'>{link.label}</Span>
                    <svg width='11' height='11' viewBox='0 0 24 24' fill='none' stroke='#9ca3af' strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
                ...
                <View className='items-center justify-center rounded-xl border border-dashed border-gray-200 py-4'>
                  <Span className='text-[10px] text-gray-300'>Chưa có liên kết</Span>
                </View>
              )}
            </View>
          </ScrollView>
          ...
          <View className='absolute inset-x-0 bottom-0 rounded-t-3xl bg-white' style={{ maxHeight: '90%' }}>
            <ScrollView>
              <View className='p-4'>
                {/* Header */}
                <View className='mb-4 flex-row items-center justify-between'>
                  <Span className='text-sm font-semibold text-gray-800'>Theo dõi sức khỏe</Span>
                  <Pressable onPress={() => setShowEdit(false)} className='h-7 w-7 items-center justify-center rounded-full bg-red-50'>
                ...
                {/* Form fields */}
                {[
                  { key: 'disease', label: 'Tên bệnh' },
                  { key: 'time', label: 'Thời gian' },
                  { key: 'place', label: 'Địa điểm' },
                  { key: 'note', label: 'Chú ý' },
                ].map(f => (
                  <View key={f.key} className='mb-3'>
                    <Span className='mb-1 text-[10px] font-medium text-gray-400'>{f.label.toUpperCase()}</Span>
                    <View className='rounded-xl border border-gray-200 bg-slate-50 px-3 py-2.5'>
                      <Span className='text-xs text-gray-400'>Nhập {f.label.toLowerCase()}...</Span>
                    </View>
                  </View>
                ))}
                ...
                <View className='mb-3'>
                  ...
                  <View className='flex-row items-center gap-2'>
                    <View className='flex-1 rounded-xl border border-gray-200 bg-slate-50 px-3 py-2.5'>
                      <Span className='text-xs text-gray-400'>Nhập tên thuốc...</Span>
                    </View>
                  ...
                  <View className='flex-row items-center gap-2'>
                    <View className='flex-1 rounded-xl border border-gray-200 bg-slate-50 px-3 py-2.5'>
                      <Span className='text-xs text-gray-400'>
                        {editForm.reminder ? editForm.reminder : 'Chưa đặt nhắc nhở...'}
                      </Span>
                    </View>
                    <Pressable onPress={() => setShowReminder(true)} className='h-10 w-10 items-center justify-center rounded-xl bg-blue-50'>
                      <svg width='18' height='18' viewBox='0 0 24 24' fill='none' stroke={PRIMARY} strokeWidth='2' strokeLinecap='round' strokeLinejoin='round'>
                ...
                {/* Actions */}
                <View className='flex-row gap-3'>
                  <Pressable onPress={() => setShowEdit(false)} className='flex-1 items-center rounded-xl border border-gray-200 bg-white py-3'>
                    <Span className='text-sm text-gray-600'>Hủy bỏ</Span>
                  </Pressable>
                  <Pressable onPress={() => setShowEdit(false)} className='flex-1 items-center rounded-xl py-3' style={{ backgroundColor: PRIMARY }}>
                    <Span className='text-sm font-semibold text-white'>Lưu kết quả</Span>
                  </Pressable>
                </View>
              </View>
            </ScrollView>
      ...
      <ScrollView className='flex-1 bg-slate-50'>
        ...
        <View className='mt-3'>
          <ScrollView
            ...
            onScroll={e => {
              ...
              setSlide(Math.min(Math.max(Math.round(x / (CARD_W + CARD_GAP)), 0), reminders.length - 1))
            }}
          ...
          >
            {reminders.map((r, i) => {
              ...
              return (
                <View key={r.id} className='relative overflow-hidden rounded-2xl bg-slate-100' style={{ width: CARD_W }}>
                  <Pressable onPress={() => setDeleteIndex(i)} className='absolute right-2 top-2 z-10 h-5 w-5 items-center justify-center rounded-full bg-white/70'>
                    <Span className='text-[10px] font-bold text-gray-400'>✕</Span>
                  </Pressable>
                  <View className='p-3'>
                    <View className='flex-row items-center gap-2'>
                      <View className={['h-7 w-7 flex-shrink-0 items-center justify-center rounded-full', r.color]}>
                        <Span className='text-[9px] font-bold text-white'>
                          {r.type === 'medicine' ? 'Rx' : r.type === 'vaccine' ? 'V' : 'Re'}
                        </Span>
                      </View>
                    ...
                    <View className='mt-2'>
                      <Span className='text-[10px] text-gray-400'>{r.scheduleLabel}</Span>
                      <Span className='text-lg font-bold text-gray-900'>{formatDate(r.date)}</Span>
                      {daysLeft === 0
                        ? <Span className='text-[11px] italic text-red-500'>Hôm nay</Span>
                        : daysLeft > 0 && daysLeft <= 3
                          ? <Span className='text-[11px] italic text-red-500'>còn {daysLeft} ngày</Span>
                          : null}
                      <Span className='mt-1 text-[11px] font-medium text-gray-700'>{r.name}</Span>
                    </View>
                  </View>
                </View>
              )
          ...
          <View className='mt-2 flex-row items-center justify-center gap-1.5'>
            {reminders.map((_, i) =>
              i === slide
                ? <View key={i} className='rounded-full bg-blue-700' style={{ width: 32, height: 6 }} />
                : <Pressable key={i} onPress={() => setSlide(i)} className='rounded-full bg-blue-200' style={{ width: 6, height: 6 }} />,
            )}
          </View>
        </View>
        ...
        <View className='mt-8 flex-row gap-4' style={{ marginHorizontal: 17 }}>
          <Pressable onPress={() => setActiveSection('tracking')} className='flex-1 items-center justify-center' style={[{ height: 44, borderRadius: 28 }, activeSection === 'tracking' ? tabActive : tabInactive]}>
            <Span style={{ color: activeSection === 'tracking' ? '#5B352B' : '#754539', fontSize: 13, fontWeight: activeSection === 'tracking' ? '500' : '400' }}>Theo dõi sức khỏe</Span>
          </Pressable>
          <Pressable onPress={() => setActiveSection('history')} className='flex-1 items-center justify-center' style={[{ height: 44, borderRadius: 28 }, activeSection === 'history' ? tabActive : tabInactive]}>
            <Span style={{ color: activeSection === 'history' ? '#5B352B' : '#754539', fontSize: 13, fontWeight: activeSection === 'history' ? '500' : '400' }}>Lịch sử khám bệnh</Span>
          </Pressable>
        </View>

        {/* Search bar + calendar */}
        <View className='mt-6 flex-row items-center gap-2' style={{ marginHorizontal: 32 }}>
          <View className='flex-1 flex-row items-center justify-between rounded-full border px-3' style={{ height: 32, borderColor: '#CAE3FF', backgroundColor: '#fff' }}>
        ...
        {activeSection === 'tracking' && (
          <View className='mb-3' style={{ marginHorizontal: 20, marginTop: 20 }}>
            {/* Section header */}
            ...
            {allTracking.map((rec, i) => (
              <View key={rec.id}>
                ...
                <View className='mb-4 rounded-2xl border border-blue-100 bg-white p-3 shadow-sm'>
                  <View className='mb-1 flex-row items-center justify-between'>
                    <Span className='text-xs font-semibold text-gray-800'>{rec.disease}</Span>
                    <Pressable
                      onPress={() => setUnfollowId(rec.id)}
                      className='rounded-full px-2 py-0.5'
                      style={{ backgroundColor: '#fee2e2' }}
                    >
                      <Span className='text-[9px] font-medium' style={{ color: '#dc2626' }}>Bỏ theo dõi</Span>
                    </Pressable>
                  </View>
                  ...
                  <Pressable onPress={() => { const rawId = rec.id.replace('copy_', ''); const extras = mockExtras[rawId] ?? { images: [], links: [] }; setDetailRecord({ id: rec.id, date: rec.date, disease: rec.disease, place: rec.place, medicine: rec.medicine, doctor: (rec as any).doctor, ...extras }) }} className='mt-2 flex-row items-center justify-center gap-1'>
                    ...
                    <Span className='text-[10px] font-medium text-blue-500'>Xem chi tiết</Span>
                  </Pressable>
                </View>
              </View>
            ))}
        ...
        {activeSection === 'history' && (
          <View className='mb-4' style={{ marginHorizontal: 20, marginTop: 20 }}>
            ...
            <View className='mb-2 flex-row items-center justify-between'>
              <View className='flex-row items-center gap-1.5'>
                <Span className='text-sm font-semibold text-gray-800'>Lịch sử khám bệnh</Span>
                {!bulkMode && !specialView && (
                  <Pressable onPress={() => setShowSectionMore(true)} className='h-6 w-6 items-center justify-center'>
              ...
              <View className='flex-row items-center gap-2'>
                {bulkMode && (
                  <Pressable onPress={() => { setBulkMode(null); setSelectedIds(new Set()) }} className='rounded-full bg-gray-100 px-2 py-0.5'>
                    <Span className='text-[10px] text-gray-500'>Hủy</Span>
                  </Pressable>
                )}
                {bulkMode && selectedIds.size > 0 && (
                  <Pressable onPress={() => setShowBulkConfirm(true)} className='rounded-full px-2 py-0.5' style={{ backgroundColor: PRIMARY }}>
                    <Span className='text-[10px] font-semibold text-white'>
                      {bulkMode === 'pdf' ? 'Xuất PDF' : 'Chia sẻ'} ({selectedIds.size})
                    </Span>
                  </Pressable>
                )}
                {specialView && (
                  <Pressable onPress={() => setSpecialView(null)} className='rounded-full bg-gray-100 px-2 py-0.5'>
            ...
            {!specialView && !bulkMode && (
              <View className='mb-3 flex-row gap-2'>
                {[
                  { label: 'Gần nhất', value: 'Gần nhất' as const },
                  { label: 'Cũ nhất', value: 'Cũ nhất' as const },
                  { label: 'Bộ lọc', value: 'Bộ lọc' as const },
                ].map(opt => (
                  <Pressable
                    key={opt.value}
                    onPress={() => opt.value === 'Bộ lọc' ? setShowFilter(true) : setSort(opt.value)}
                    className={['rounded-full px-3 py-1', sort === opt.value ? 'bg-blue-600' : 'border border-gray-200 bg-white']}
                  >
                    <Span className={['text-[10px] font-medium', sort === opt.value ? 'text-white' : 'text-gray-500']}>{opt.label}</Span>
                  </Pressable>
                ))}
              </View>
            )}
            ...
            {specialView && (() => {
              const specIds = specialView === 'hidden' ? hiddenIds : deletedIds
              const specRecs = historyRecords.filter(r => specIds.has(r.id))
              if (specRecs.length === 0)
                return <Span className='text-center text-xs text-gray-400 py-8'>Không có hồ sơ nào</Span>
              return specRecs.map(rec => (
                <View key={rec.id} className='mb-4 rounded-2xl border border-gray-200 bg-white p-3 shadow-sm'>
                  <View className='mb-1 flex-row items-center justify-between'>
                    <Span className='text-xs font-semibold text-gray-800'>{rec.disease}</Span>
                  ...
                  <Pressable onPress={() => setRestoreId(rec.id)} className='mt-2 flex-row items-center justify-center gap-1'>
                    ...
                    <Span className='text-[10px] font-medium' style={{ color: PRIMARY }}>Khôi phục</Span>
                  </Pressable>
                </View>
              ))
            })()}
            ...
            {!specialView && filteredHistory.map(rec => {
              ...
              return (
                <View key={rec.id}>
                  <View className='mb-1.5 flex-row items-center gap-2'>
                    {bulkMode && (
                      <Pressable onPress={() => setSelectedIds(prev => { const n = new Set(prev); n.has(rec.id) ? n.delete(rec.id) : n.add(rec.id); return n })}>
                        <View className='h-4 w-4 items-center justify-center rounded-sm border' style={{ borderColor: isSelected ? PRIMARY : '#d1d5db', backgroundColor: isSelected ? PRIMARY : '#fff' }}>
                  ...
                  <View
                    ...
                    style={isSelected ? { borderColor: PRIMARY, borderWidth: 1.5 } : {}}
                  >
                    <View className='mb-1 flex-row items-center justify-between'>
                      <Span className='text-xs font-semibold text-gray-800'>{rec.disease}</Span>
                      {!bulkMode && (
                        <View className='flex-row items-center gap-1.5'>
                          <Pressable onPress={() => setRecordMoreId(rec.id)} className='h-5 w-5 items-center justify-center'>
                            <Span className='text-[11px] font-bold text-gray-400'>···</Span>
                          </Pressable>
                          <Pressable
                            onPress={() => isFollowing ? setUnfollowHistoryId(rec.id) : setFollowConfirmId(rec.id)}
                            className='rounded-full px-2 py-0.5'
                            style={isFollowing ? { backgroundColor: '#dbeafe' } : { backgroundColor: '#f1f5f9' }}
                          >
                            <Span className='text-[9px] font-medium' style={{ color: isFollowing ? '#1d4ed8' : '#64748b' }}>
