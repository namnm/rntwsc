import type {
  ClassName,
  ClassNameState,
  ClassNameWithSelector,
  Style,
  StyleSingle,
} from 'rntwsc/tw/class-name'
import { omitEmptyObject } from 'rntwsc/tw/lib/class-name-to-native'
import type { ClassNameToStylesOptions } from 'rntwsc/tw/lib/class-name-to-styles'
import { classNameToStyles } from 'rntwsc/tw/lib/class-name-to-styles'
import { normalizeStyle } from 'rntwsc/tw/lib/normalize-style'

type Options = Partial<
  Omit<ClassNameToStylesOptions, 'className'> &
    Pick<ClassNameToStylesOptions, 'variables' | 'dimensions'> & {
      state: ClassNameState
      style: Style
    }
>

export const runtimeStyle = (
  className: ClassName,
  { state, variables, dimensions, style, onSelector, ...options }: Options = {},
): StyleSingle | undefined => {
  const styles = classNameToStyles({
    className,
    onSelector:
      onSelector ||
      (selector =>
        defaultOnSelector({
          className: selector,
          state,
        })),
    variables,
    dimensions,
    ...options,
  })

  if (Array.isArray(style)) {
    style = style.flat(Infinity as 0).filter(v => v)
    style = Object.assign({}, ...style)
  }

  const flatten = Object.assign({}, ...styles, style)
  normalizeStyle(flatten)

  return omitEmptyObject(flatten)
}

type DefaultOnSelectorOptions = Pick<Options, 'state'> & {
  className: ClassNameWithSelector
}
const defaultOnSelector = ({
  className: { selector, style },
  state,
}: DefaultOnSelectorOptions) => {
  if (selector === true) {
    return style
  }
  if (state && state[selector]) {
    return style
  }
  return
}
