...
`optimizeDeps.include` also lists `use-sync-external-store` and `use-sync-external-store/shim` - react-i18next (pulled in by rntwsc's own packages/core/i18n, used by Span/View/etc for RTL detection) ships that package's CJS shim; without forcing it through esbuild's pre-bundler, the browser loads it as raw ESM and gets no named exports from a CJS file. Unlike the runtime deps in the next section, these two are resolved from the consuming project's own node_modules, not aliased from inside rntwsc's install - a consumer that pulls in rntwsc's i18n already declares `react-i18next`/`i18next` itself (same as `react`/`react-dom` above), so `use-sync-external-store` follows the same by-name pattern.
