...
Metro also falls back to a module's browser file, not just packages/devtools/metro-config's own stock native/ios/android resolution: packages/devtools/metro-config/index.ts wraps its resolveRequest so that whenever Metro's own resolution lands on a bare (non-native-suffixed) file, it checks for a sibling browser file and redirects there instead. This is why cache, fetch, and graphql (whose default implementation is server-only, RSC-based) need no native file at all - their browser implementation is plain, client-safe code that works unchanged under React Native too.
...
Two web variants exist: browser (Next's client bundle) and web (a plain Vite SPA, no server - named to match the wider React Native ecosystem's own `.web.js` convention). See web-variant.md for the latter - most modules need no web file at all, since web falls back to browser automatically wherever no web-specific file exists. Default (server) is the broadest, then browser, then native.
